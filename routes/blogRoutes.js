...
router.get('/', async (req, res) => {
  try {
    ...
    // Base query
    let query = { status: 'published' }; // Only show published blogs by default
    
    // If admin and status filter provided, apply it
    if (isAdmin && req.query.status) {
      if (req.query.status === 'all') {
        delete query.status; // Remove status filter to show all posts
      } else {
...
router.post('/', protect, upload.single('featuredImage'), async (req, res) => {
  try {
    ...
    if (req.file) {
      featuredImagePath = `/uploads/blog/${req.file.filename}`;
